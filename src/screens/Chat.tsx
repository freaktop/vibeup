...
export default function Chat({ profileId }: ChatProps) {
  ...
  useEffect(() => {
    ...
    if (!auth) {
      console.log('[Chat] Firebase auth not configured, using demo mode');
      const uid = 'demo-user-vibeup';
      if (profileId) {
        const mId = matchIdFor(uid, profileId);
        setMatchId(mId);
    ...
    <div className="chat-container">
      ...
      <div className="chat-messages">
        ...
        {messages.map((message) => (
          <div
            key={message.id}
            className={`chat-message ${message.senderId === (getCurrentUid() ?? 'me') ? 'sent' : 'received'}`}
          >
