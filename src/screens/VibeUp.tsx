...
export default function VibeUp() {
  ...
  useEffect(() => {
    ...
    if (!auth) {
      console.log('[VibeUp] Firebase auth not configured, using demo mode');
      const uid = 'demo-user-vibeup';
      unsubProfiles = listenProfiles((p) => {
        enrichProfilesWithDistance(p).then(setAllProfiles);
      });
      unsubLikedMe = listenWhoLikedMe(uid, (ids) => {
        setWhoLikedMeIds(ids);
        setIsLoading(false);
      });
      unsubSwipes = listenMySwipes(uid, (swipes: Record<string, SwipeType>) => {
