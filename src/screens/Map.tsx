...
export default function Map() {
  ...
  useEffect(() => {
    ...
    if (!auth) {
      console.log('[Map] Firebase auth not configured, using demo mode');
      setIsLoading(true);
      unsubProfiles = listenProfiles((profileRows) => {
        const enriched = profileRows.map((p) => {
          ...
          const lng = p.lng ?? getCityCoords(currentCity)?.lng;
          return { ...p, lat, lng };
  ...
  return (
    <div className="map-container">
      ...
      <ReportModal
        ...
        profileName={selectedProfile ? `${(selectedProfile as any).name}, ${(selectedProfile as any).age}` : undefined}
