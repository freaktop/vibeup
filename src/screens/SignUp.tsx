...
export default function SignUp({ onSignup, onSwitchToLogin }: SignUpProps) {
  ...
  const handleSignup = async () => {
    if (!email.trim() || !password.trim() || !name.trim() || !dob) {
      setError('Please fill in all fields');
      return;
    }

    if (!isOver18(dob)) {
      setError('You must be at least 18 years old to use VibeUp');
      return;
    }
    ...
    <div className="auth-container">
      ...
      >
        ...
        <div className="auth-input-group">
          ...
          />
          {dob && !isOver18(dob) && (
            <div className="auth-field-error" style={{ color: '#e74c3c', fontSize: 12, marginTop: 4 }}>You must be at least 18 years old</div>
          )}
        </div>
