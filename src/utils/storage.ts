...
export const storage = {
  ...
  // Chat messages (by matchId)
  getChatMessages(matchId: string): any[] {
    try {
      const key = `@vibeup:chat_${matchId}`;
  ...
  saveChatMessages(matchId: string, messages: any[]): void {
    try {
      const key = `@vibeup:chat_${matchId}`;
  ...
  getReports(): any[] {
    try {
      const data = safeGetItem('@vibeup:reports');
      return data ? JSON.parse(data) : [];
    } catch (error) {
  ...
  addReport(report: any): void {
    try {
      ...
      reports.push(report);
      safeSetItem('@vibeup:reports', JSON.stringify(reports));
    } catch (error) {
